...
# PyInstaller spec — produce telephony-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller telephony-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/telephony_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/telephony_mcp", "telephony_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
